# etl_load_dw_sqlserver_fixed.py  (PATCHED: debug + delete-all-before-insert)
import os
import time
from datetime import datetime, timedelta
import pyodbc
import pandas as pd
from dotenv import load_dotenv
from sqlalchemy import create_engine, text

load_dotenv()

SRC = {
    "host": os.getenv("SRC_HOST", "172.24.56.35,1433"),
    "user": os.getenv("SRC_USER", "Inteligencia"),
    "pass": os.getenv("SRC_PASS", "Rock2213#"),
    "db":   os.getenv("SRC_DB", "PROJECT_MANAGE")
}
DST = {
    "host": os.getenv("DST_HOST", "172.24.84.67,1433"),
    "user": os.getenv("DST_USER", "admin_kiry2"),
    "pass": os.getenv("DST_PASS", "1234"),
    "db":   os.getenv("DST_DB", "PROJECT_SUPPORT_SYSTEM")
}

OVERRIDE_START = os.getenv("SRC_START_COL", "").strip() or None
OVERRIDE_END   = os.getenv("SRC_END_COL", "").strip() or None

DRIVER = "ODBC Driver 17 for SQL Server"

def create_sqlalchemy_engine(host, user, password, database):
    if "," in host:
        server, port = host.split(",", 1)
        host_formatted = f"{server}:{port}"
    else:
        host_formatted = host
    conn_str = f"mssql+pyodbc://{user}:{password}@{host_formatted}/{database}?driver=ODBC+Driver+17+for+SQL+Server"
    return create_engine(conn_str, fast_executemany=True)

def conn_pyodbc(cfg):
    conn_str = f"DRIVER={{{DRIVER}}};SERVER={cfg['host']};DATABASE={cfg['db']};UID={cfg['user']};PWD={cfg['pass']}"
    return pyodbc.connect(conn_str, autocommit=True)

def ensure_etl_audit(dst_cn):
    cur = dst_cn.cursor()
    cur.execute("""
    IF OBJECT_ID('dbo.etl_audit') IS NULL
    CREATE TABLE dbo.etl_audit (
      run_id DATETIME PRIMARY KEY,
      src_rows_time_log INT,
      src_rows_financials INT,
      src_rows_issue INT,
      dst_rows_dim_client INT,
      dst_rows_dim_team INT,
      dst_rows_dim_project INT,
      dst_rows_fact_project INT,
      duration_seconds FLOAT,
      status VARCHAR(20),
      notes NVARCHAR(1000)
    );
    """)
    try:
        dst_cn.commit()
    except:
        pass

def find_column_names(src_engine, table_name='project'):
    try:
        q = text("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = :table")
        rows = src_engine.execute(q, {"table": table_name}).fetchall()
        cols = [r[0].lower() for r in rows]
        return set(cols)
    except Exception:
        return set()

def choose_column(cols_set, candidates):
    for c in candidates:
        if c.lower() in cols_set:
            return c.lower()
    return None

def upsert_dim_date(dst_cn, start_date, end_date):
    cur = dst_cn.cursor()
    rows = []
    d = start_date
    while d <= end_date:
        date_id = int(d.strftime("%Y%m%d"))
        rows.append((date_id, d, d.year, (d.month-1)//3+1, d.month, d.day, d.strftime("%A")))
        d += timedelta(days=1)
    tmp = "#tmp_dates"
    cur.execute(f"IF OBJECT_ID('tempdb..{tmp}') IS NOT NULL DROP TABLE {tmp}")
    cur.execute(f"CREATE TABLE {tmp} (date_id INT, full_date DATE, [year] INT, quarter INT, [month] INT, [day] INT, weekday VARCHAR(20))")
    if rows:
        cur.fast_executemany = True
        cur.executemany(f"INSERT INTO {tmp} VALUES (?, ?, ?, ?, ?, ?, ?)", rows)
    cur.execute(f"""
    MERGE INTO dim_date AS T
    USING {tmp} AS S
    ON T.date_id = S.date_id
    WHEN NOT MATCHED BY TARGET THEN
      INSERT (date_id, full_date, year, quarter, month, day, weekday)
      VALUES (S.date_id, S.full_date, S.year, S.quarter, S.month, S.day, S.weekday);
    """)
    cur.execute(f"DROP TABLE {tmp}")
    try:
        dst_cn.commit()
    except:
        pass

def upsert_simple_table(dst_cn, df, table_name, key):
    if df is None or df.empty:
        return 0
    cur = dst_cn.cursor()
    cols = df.columns.tolist()
    tmp = f"#tmp_{table_name}"
    cur.execute(f"IF OBJECT_ID('tempdb..{tmp}') IS NOT NULL DROP TABLE {tmp}")
    create = "CREATE TABLE " + tmp + " (" + ",".join([f"[{c}] NVARCHAR(4000)" for c in cols]) + ")"
    cur.execute(create)
    rows = [tuple(str(x) if x is not None else None for x in row) for row in df.values.tolist()]
    placeholders = "(" + ",".join(["?"]*len(cols)) + ")"
    if rows:
        cur.fast_executemany = True
        cur.executemany(f"INSERT INTO {tmp} VALUES {placeholders}", rows)
    merge_sql = f"""
    MERGE INTO {table_name} AS T
    USING {tmp} AS S
    ON T.{key} = S.{key}
    WHEN NOT MATCHED BY TARGET THEN
      INSERT ({','.join(['['+c+']' for c in cols])}) VALUES ({','.join(['S.'+'['+c+']' for c in cols])});
    """
    cur.execute(merge_sql)
    cur.execute(f"DROP TABLE {tmp}")
    try:
        dst_cn.commit()
    except:
        pass
    return len(df)

def main():
    start_time = time.time()
    run_ts = datetime.now()
    status = "OK"
    notes = ""
    src_engine = None
    dst_engine = None
    src_cn = None
    dst_cn = None
    try:
        src_engine = create_sqlalchemy_engine(SRC['host'], SRC['user'], SRC['pass'], SRC['db'])
        dst_engine = create_sqlalchemy_engine(DST['host'], DST['user'], DST['pass'], DST['db'])
        src_cn = conn_pyodbc(SRC)
        dst_cn = conn_pyodbc(DST)
        print("Connected to source and destination.")
        ensure_etl_audit(dst_cn)

        # detect or override date columns
        start_col = OVERRIDE_START
        end_col = OVERRIDE_END
        if not start_col or not end_col:
            src_cols = find_column_names(src_engine, 'project')
            start_candidates = ['start_date','startdate','start','fecha_inicio','begin_date','inicio','start_dt']
            end_candidates   = ['end_date','enddate','end','fecha_fin','finish_date','finish','end_dt']
            if not start_col:
                start_col = choose_column(src_cols, start_candidates)
            if not end_col:
                end_col = choose_column(src_cols, end_candidates)

        if start_col and end_col:
            print(f"Using source date columns: start='{start_col}', end='{end_col}'")
        else:
            print("No standard start/end columns found in source 'project' table. Will use defaults for date range.")

        # build dim_date
        if start_col and end_col:
            q = f"SELECT MIN([{start_col}]) AS min_s, MAX([{end_col}]) AS max_e FROM project"
            r = pd.read_sql(q, src_engine).iloc[0]
            min_date = r['min_s']
            max_date = r['max_e']
        else:
            min_date = datetime(2023,1,1)
            max_date = datetime.now()

        if pd.isna(min_date) or pd.isna(max_date):
            min_date = datetime(2023,1,1)
            max_date = datetime.now()

        min_date = pd.to_datetime(min_date) - timedelta(days=30)
        max_date = pd.to_datetime(max_date) + timedelta(days=30)
        upsert_dim_date(dst_cn, min_date.date(), max_date.date())

        # dims
        df_client = pd.read_sql("SELECT client_id, client_name, industry, tier FROM client", src_engine)
        n_client = upsert_simple_table(dst_cn, df_client, "dim_client", "client_id")

        df_team = pd.read_sql("SELECT team_id, team_name, description FROM team", src_engine)
        n_team = upsert_simple_table(dst_cn, df_team, "dim_team", "team_id")

        df_proj = pd.read_sql("SELECT project_id, project_name, priority, budget FROM project", src_engine)
        n_proj = upsert_simple_table(dst_cn, df_proj, "dim_project", "project_id")

        # source counts
        src_tl = int(pd.read_sql("SELECT COUNT(*) AS cnt FROM time_log", src_engine).iloc[0]['cnt'])
        src_fin = int(pd.read_sql("SELECT COUNT(*) AS cnt FROM financials", src_engine).iloc[0]['cnt'])
        src_issue = int(pd.read_sql("SELECT COUNT(*) AS cnt FROM [issue]", src_engine).iloc[0]['cnt'])

        # aggregations
        df_hours = pd.read_sql("""
            SELECT p.project_id,
                ISNULL(SUM(t.hours),0) AS total_hours,
                ISNULL(SUM(t.cost),0) AS total_cost
            FROM project p
            LEFT JOIN time_log t ON p.project_id = t.project_id
            GROUP BY p.project_id
        """, src_engine)

        df_rev = pd.read_sql("SELECT project_id, ISNULL(SUM(CASE WHEN type='revenue' THEN amount ELSE 0 END),0) AS total_revenue FROM financials GROUP BY project_id", src_engine)
        df_err = pd.read_sql("SELECT project_id, COUNT(*) AS total_errors FROM [issue] GROUP BY project_id", src_engine)

        if start_col and end_col:
            q_dates = f"SELECT project_id, [{start_col}] AS start_date, [{end_col}] AS end_date, client_id, team_id FROM project"
        else:
            q_dates = "SELECT project_id, NULL AS start_date, NULL AS end_date, client_id, team_id FROM project"
        df_dates = pd.read_sql(q_dates, src_engine)

        # normalize dates -> ids
        df_dates['start_date'] = pd.to_datetime(df_dates['start_date'], errors='coerce').fillna(pd.to_datetime('2023-01-01'))
        df_dates['end_date']   = pd.to_datetime(df_dates['end_date'], errors='coerce').fillna(pd.to_datetime('2023-01-01'))
        df_dates['start_date_id'] = df_dates['start_date'].dt.strftime("%Y%m%d").astype(int)
        df_dates['end_date_id']   = df_dates['end_date'].dt.strftime("%Y%m%d").astype(int)

        # debug prints: shapes
        print("DEBUG: df_hours rows:", None if df_hours is None else len(df_hours))
        print("DEBUG: df_rev rows:", None if df_rev is None else len(df_rev))
        print("DEBUG: df_err rows:", None if df_err is None else len(df_err))
        print("DEBUG: df_dates rows:", None if df_dates is None else len(df_dates))

        # ensure not empty
        if df_hours is None or df_hours.empty:
            df_hours = pd.DataFrame(columns=['project_id','total_hours','total_cost'])
        if df_rev is None or df_rev.empty:
            df_rev = pd.DataFrame(columns=['project_id','total_revenue'])
        if df_err is None or df_err.empty:
            df_err = pd.DataFrame(columns=['project_id','total_errors'])

        df_fact = df_hours.merge(df_rev, on="project_id", how="left").merge(df_err, on="project_id", how="left").merge(df_dates, on="project_id", how="left")
        print("DEBUG: df_fact rows (merged):", len(df_fact))

        df_fact["total_revenue"] = df_fact["total_revenue"].fillna(0)
        df_fact["total_errors"] = df_fact["total_errors"].fillna(0).astype(int)
        df_fact["total_hours"] = df_fact["total_hours"].fillna(0)
        df_fact["total_cost"] = df_fact["total_cost"].fillna(0)

        rows = []
        for _, r in df_fact.iterrows():
            rows.append((int(r.project_id), int(r.client_id), int(r.team_id), int(r.start_date_id), int(r.end_date_id),
                         float(r.total_hours), float(r.total_cost), float(r.total_revenue), int(r.total_errors)))

        dst_cur = dst_cn.cursor()
        if rows:
            # --- SAFER: delete all facts, then insert all rows ---
            print("DEBUG: deleting all rows from fact_project before insert...")
            dst_cur.execute("DELETE FROM fact_project")
            try:
                dst_cn.commit()
            except:
                pass

            insert_sql = """INSERT INTO fact_project (project_id, client_id, team_id, start_date_id, end_date_id, total_hours, total_cost, total_revenue, total_errors)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
            dst_cur.fast_executemany = True
            dst_cur.executemany(insert_sql, rows)
            try:
                dst_cn.commit()
            except Exception as e:
                print("Warning during commit of inserts:", e)

        duration = time.time() - start_time
        try:
            dst_cn.cursor().execute(
                "INSERT INTO etl_audit (run_id, src_rows_time_log, src_rows_financials, src_rows_issue, dst_rows_dim_client, dst_rows_dim_team, dst_rows_dim_project, dst_rows_fact_project, duration_seconds, status, notes) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                (run_ts, src_tl, src_fin, src_issue, n_client, n_team, n_proj, len(rows), duration, "OK", f"Used start_col={start_col} end_col={end_col}")
            )
            try:
                dst_cn.commit()
            except:
                pass
        except Exception as e:
            print("Warning writing etl_audit:", e)

        print("ETL finalizado. Projects processed:", len(rows))
    except Exception as e:
        status = "ERROR"
        notes = str(e)
        print("ERROR ETL:", e)
        try:
            if dst_cn is not None:
                dst_cn.cursor().execute("INSERT INTO etl_audit (run_id, duration_seconds, status, notes) VALUES (?,?,?,?)", (run_ts, time.time()-start_time, status, notes))
                try:
                    dst_cn.commit()
                except:
                    pass
        except:
            pass

if __name__ == "__main__":
    main()
