...
def generate(clean=False):
    ...
    for p in project_rows:
        # project_id is first column
        pid_val = p[0]
        # If exists, skip insert; if not, insert
        # Construimos params: primero el valor para el IF NOT EXISTS, luego los 10 valores del INSERT
        params = (pid_val,) + p   # p es la tupla (project_id, project_name, ..., current_stage_id)
        cur.execute(
