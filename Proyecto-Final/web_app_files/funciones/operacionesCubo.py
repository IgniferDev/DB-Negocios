...
def dice_subset(df: pd.DataFrame, anios=None, regiones=None, productos=None, canales=None) -> pd.DataFrame:
...
def pivot_anio_region(df: pd.DataFrame) -> pd.DataFrame:
    return pd.pivot_table(df, values="Ventas", index="Año", columns="Región", aggfunc="sum", margins=True)
